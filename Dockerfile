...
RUN userhome=$(eval echo ~${DEVELOPER_USERNAME}) \
 ...
 && mkdir -p /workspaces/myblog \
 && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /workspaces/myblog \
 && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /usr/local/bundle/*

# Add the app's "bin/" directory to PATH:
ENV PATH=/workspaces/myblog/bin:$PATH

# Set the app path as the working directory:
WORKDIR /workspaces/myblog
...
COPY --chown=${DEVELOPER_USERNAME} Gemfile* /workspaces/myblog/
...
COPY --from=testing /workspaces/myblog/ /workspaces/myblog/
...
COPY --chown=${DEVELOPER_USERNAME} . /workspaces/myblog/
...
# final destination at /workspaces/myblog:
COPY --from=builder --chown=nobody:nogroup /workspaces/myblog /workspaces/myblog

# Set the container user to 'nobody':
USER root

# Set the RAILS and PORT default values:
ENV HOME=/workspaces/myblog \
...
ENTRYPOINT [ "/workspaces/myblog/bin/entrypoint" ]
